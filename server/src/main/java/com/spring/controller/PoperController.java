...
public class PoperController extends BaseController{
    ...
    public String save()
    {
      ...
      /*  jieguo.put("shifoujige", total >= 60 ? "及格" : "不及格");*/
        ...
        long insertId = Query.make("wenjuanjieguo").insert(jieguo);
        return jsonResult(insertId);
    }

    @RequestMapping("/savePoper2")
    public String save1()
    {
        if(!checkLogin())
        {
        ...
        /*  jieguo.put("shifoujige", total >= 60 ? "及格" : "不及格");*/
        jieguo.put("kaoshishichang", Request.get("time"));
        long insertId = Query.make("ceshichengji").insert(jieguo);
        return jsonResult(insertId);
    }
}
