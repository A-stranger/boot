 ...
 * 测试试题 */
@Controller
public class CeshishitiController extends BaseController
{
    @Autowired
    private CeshishitiMapper dao;
    @Autowired
    private CeshishitiService service;
     ...
     *
     */
    @RequestMapping("/ceshishiti_list")
    public String list()
    {
        ...
        Example example = new Example(Ceshishiti.class); //  创建一个扩展搜索类
        ...
        List<Ceshishiti> list = service.selectPageExample(example , page , pagesize);   // 获取当前页的行数
     ...
     * 发布人列表
     */
    @RequestMapping("/ceshishiti_list_faburen")
    public String listfaburen()
    {
        ...
        page = Math.max(1 , page); // 注释同list
            ...
            List<Ceshishiti> list = service.selectPageExample(example , page , pagesize);
                    assign("totalCount" , request.getAttribute("totalCount"));
        assign("list" , list);
    ...
    @RequestMapping("/ceshishiti_updt")
    public String updt()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Ceshishiti mmm = service.find(id);
        assign("mmm" , mmm);
        assign("updtself" , 0);
     ...
     * @return
     */
    @RequestMapping("/ceshishitiinsert")
    public String insert()
    {
        _var = new LinkedHashMap(); // 重置数据
        String tmp="";
        Ceshishiti post = new Ceshishiti();  // 创建实体类
        // 设置前台提交上来的数据到实体类中
        ...
        Query.execute("INSERT INTO logs(module,operationtype,operationcontent,cx,username) values('测试试题' , '新增' , '编号："+request.getParameter("bianhao")+"<br>标题："+request.getParameter("biaoti")+"<br>名称："+request.getParameter("mingcheng")+"<br>试题题目："+request.getParameter("shititimu")+"' , '"+request.getSession().getAttribute("cx")+"' , '"+request.getSession().getAttribute("username")+"')");
    ...
    * @return
    */
    @RequestMapping("/ceshishitiupdate")
    public String update()
    {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Ceshishiti post = new Ceshishiti();
        // 将前台表单数据填充到实体类
        ...
        int charuid = post.getId().intValue();
        Query.execute("INSERT INTO logs(module,operationtype,operationcontent,cx,username) values('测试试题' , '更新' , '编号："+request.getParameter("bianhao")+"<br>标题："+request.getParameter("biaoti")+"<br>名称："+request.getParameter("mingcheng")+"<br>试题题目："+request.getParameter("shititimu")+"' , '"+request.getSession().getAttribute("cx")+"' , '"+request.getSession().getAttribute("username")+"')");
     ...
     *  后台详情
     */
    @RequestMapping("/ceshishiti_detail")
    public String detail()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Ceshishiti map = service.find(id);  // 根据前台url 参数中的id获取行数据
    ...
    *  删除
    */
    @RequestMapping("/ceshishiti_delete")
    public String delete()
    {
        ...
        HashMap map = Query.make("ceshishiti").find(id);

                service.delete(id);// 根据id 删除某行数据
        Query.execute("INSERT INTO logs(module,operationtype,operationcontent,cx,username) values('测试试题' , '删除' , '编号："+map.get("bianhao")+"<br>标题："+map.get("biaoti")+"<br>名称："+map.get("mingcheng")+"<br>试题题目："+map.get("shititimu")+"' , '"+request.getSession().getAttribute("cx")+"' , '"+request.getSession().getAttribute("username")+"')");
