 ...
 * 测试结果 */
@Controller
public class CeshijieguoController extends BaseController
{
    @Autowired
    private CeshijieguoMapper dao;
    @Autowired
    private CeshijieguoService service;
     ...
     *
     */
    @RequestMapping("/ceshijieguo_list")
    public String list()
    {
        ...
        Example example = new Example(Ceshijieguo.class); //  创建一个扩展搜索类
        ...
        List<Ceshijieguo> list = service.selectPageExample(example , page , pagesize);   // 获取当前页的行数
     ...
     * 发布人列表
     */
    @RequestMapping("/ceshijieguo_list_faburen")
    public String listfaburen()
    {
        ...
        page = Math.max(1 , page); // 注释同list
            ...
            List<Ceshijieguo> list = service.selectPageExample(example , page , pagesize);
                    assign("totalCount" , request.getAttribute("totalCount"));
        assign("list" , list);
     ...
     * 考试人列表
     */
    @RequestMapping("/ceshijieguo_list_kaoshiren")
    public String listkaoshiren()
    {
        ...
        @RequestMapping("/ceshijieguo_add")
    public String add()
    {
        _var = new LinkedHashMap(); // 重置数据
    ...
    @RequestMapping("/ceshijieguo_updt")
    public String updt()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Ceshijieguo mmm = service.find(id);
        assign("mmm" , mmm);
        assign("updtself" , 0);
     ...
     * @return
     */
    @RequestMapping("/ceshijieguoinsert")
    public String insert()
    {
        _var = new LinkedHashMap(); // 重置数据
        String tmp="";
        Ceshijieguo post = new Ceshijieguo();  // 创建实体类
        // 设置前台提交上来的数据到实体类中
    ...
    * @return
    */
    @RequestMapping("/ceshijieguoupdate")
    public String update()
    {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Ceshijieguo post = new Ceshijieguo();
        // 将前台表单数据填充到实体类
    ...
    *  删除
    */
    @RequestMapping("/ceshijieguo_delete")
    public String delete()
    {
        ...
        HashMap map = Query.make("ceshijieguo").find(id);

                service.delete(id);// 根据id 删除某行数据
        Query.execute("INSERT INTO logs(module,operationtype,operationcontent,cx,username) values('测试结果' , '删除' , '编号："+map.get("bianhao")+"<br>标题："+map.get("biaoti")+"<br>名称："+map.get("mingcheng")+"<br>考试编号："+map.get("kaoshibianhao")+"<br>试题题目："+map.get("shititimu")+"<br>类型："+map.get("leixing")+"<br>答案："+map.get("daan")+"<br>选项："+map.get("zimu")+"' , '"+request.getSession().getAttribute("cx")+"' , '"+request.getSession().getAttribute("username")+"')");
