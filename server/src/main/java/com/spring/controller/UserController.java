...
public class UserController extends BaseController{
    ...
    private YongrendanweiService yongrendanweiService;
        @Resource
    private GaoxiaoguanliyuanService gaoxiaoguanliyuanService;
    ...
    {
        //"".equals(username) 相当于 username.equals("")，使用后者用户名为空会抛出异常
        ...
        if(isAjax() && request.getParameter("captchToken") != null ){
            //如果传来的验证码不为空，则使用des算法跟密钥解密得到验证码，否则从session中取
        ...
        //先判断页面表单传的验证码是否为空，为空则将空值赋给pagerandom,否则将获取到的值赋给它
        String pagerandom = request.getParameter("pagerandom") == null ? "" : request.getParameter("pagerandom");

        //判断页面表单传来的字符串不为空且和生成的验证码不等
        if (request.getParameter("a") != null && !pagerandom.equals(random)) {
            return showError("验证码不正确" , 20);
        }

        if (cx.equals("管理员")) {
            //按照账号密码在数据库中找，有的话把数据库中参数传给user对象
        ...
        if (cx.equals("学生")) {
            ...
            session.setAttribute("login" , cx);
                        ...
                        session.setAttribute("shifoujiuye", user.getShifoujiuye());
        ...
        if (cx.equals("用人单位")) {
            Yongrendanwei user = yongrendanweiService.login(username , pwd);
            if(user == null){
                return showError("用户名或密码错误");
            }
            ...
            session.setAttribute("login" , cx);
                        ...
                        session.setAttribute("mima", user.getMima());
                        session.setAttribute("danweimingcheng", user.getDanweimingcheng());
                        session.setAttribute("fuzeren", user.getFuzeren());
                        session.setAttribute("lianxidianhua", user.getLianxidianhua());
                        session.setAttribute("danweidizhi", user.getDanweidizhi());
                        session.setAttribute("zizhizhengming", user.getZizhizhengming());
                        session.setAttribute("addtime", user.getAddtime());
                                    JSONObject jsonObject = JSON.parseObject(JSON.toJSONString(user));
            jsonObject.put("cx" , session.getAttribute("cx"));
            jsonObject.put("username" , session.getAttribute("username"));
        ...
        if (cx.equals("高校管理员")) {
            Gaoxiaoguanliyuan user = gaoxiaoguanliyuanService.login(username , pwd);
            if(user == null){
                return showError("用户名或密码错误");
            }
                        session.setAttribute("id" , user.getId());
            ...
            session.setAttribute("login" , cx);
                        session.setAttribute("zhanghao", user.getZhanghao());
                        session.setAttribute("mima", user.getMima());
                        session.setAttribute("gaoxiaomingcheng", user.getGaoxiaomingcheng());
                        session.setAttribute("fuzeren", user.getFuzeren());
                        session.setAttribute("lianxidianhua", user.getLianxidianhua());
                        session.setAttribute("gaoxiaodizhi", user.getGaoxiaodizhi());
                        session.setAttribute("gaoxiaotupian", user.getGaoxiaotupian());
                        session.setAttribute("addtime", user.getAddtime());
                                    JSONObject jsonObject = JSON.parseObject(JSON.toJSONString(user));
            jsonObject.put("cx" , session.getAttribute("cx"));
            jsonObject.put("username" , session.getAttribute("username"));
    ...
    public String tokenLogin()
    {
        ...
        if(cx.equals("用人单位"))
        {
            user = yongrendanweiService.find(tokenInfo.get("valueid"));
        }
        if(cx.equals("高校管理员"))
        {
            user = gaoxiaoguanliyuanService.find(tokenInfo.get("valueid"));
        }
        if(user == null){
    ...
    public String editPassword()
    {
        ...
        if (cx.equals("用人单位")) {
            Yongrendanwei user = yongrendanweiService.login(username , oldPassword);
            if(user == null){
                return showError("原密码不正确");
            }
            yongrendanweiService.updatePassword(user.getId() , newPwd);
        }
        if (cx.equals("高校管理员")) {
            Gaoxiaoguanliyuan user = gaoxiaoguanliyuanService.login(username , oldPassword);
            if(user == null){
                return showError("原密码不正确");
            }
            gaoxiaoguanliyuanService.updatePassword(user.getId() , newPwd);
        }
